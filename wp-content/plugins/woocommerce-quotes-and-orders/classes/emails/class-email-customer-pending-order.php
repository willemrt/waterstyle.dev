 ...
 *
 * @class 		WC_Email_Inquiry_Customer_Pending_Order
 * @version		2.0.0
 * @package		WooCommerce/Classes/Emails
 * @author 		a3rev
 * @extends 	WC_Email
 */
class WC_Email_Inquiry_Customer_Pending_Order extends WC_Email {
	...
	function __construct() {
		...
		$this->title 			= __( 'Pending Order', 'wc_email_inquiry' );
		$this->description		= __( 'This is the Order details sent to the customer after they complete and send order.', 'wc_email_inquiry' );

		$this->heading 			= __( 'Thank you for order', 'wc_email_inquiry' );
		if ( version_compare( WC_VERSION, '2.1', '<' ) ) {
			$this->subject      = __( 'Your {blogname} order receipt from {order_date}', 'wc_email_inquiry' );
		} else {
			$this->subject      = __( 'Your {site_title} order receipt from {order_date}', 'wc_email_inquiry' );
		}
		
		$this->email_message 	= $this->get_option( 'email_message', __( 'Your order has been received and is now being processed. Your order details are shown below for your reference.', 'wc_email_inquiry' ) );
		...
		if ( version_compare( WC_VERSION, '2.5.0', '>=' ) ) {
			$this->template_html 	= 'emails/customer-pending-order_2.5.php';
			$this->template_plain 	= 'emails/plain/customer-pending-order_2.5.php';
		} elseif ( version_compare( WC_VERSION, '2.4.0', '>=' ) ) {
			$this->template_html 	= 'emails/customer-pending-order_2.4.php';
			$this->template_plain 	= 'emails/plain/customer-pending-order_2.2.php';
		} elseif ( version_compare( WC_VERSION, '2.0.0', '>' ) ) {
			$this->template_html 	= 'emails/customer-pending-order_2.1.php';
			$this->template_plain 	= 'emails/plain/customer-pending-order_2.2.php';
		}
	...
	function get_email_message() {
		return apply_filters( 'woocommerce_email_order_message_' . $this->id, $this->format_string( $this->email_message ), $this->object );
	}
    ...
    function init_form_fields() {
    	$this->form_fields = array(
			...
			'email_message' => array(
				'title' 		=> __( 'Email Message', 'woocommerce' ),
				'type' 			=> 'textarea',
				'description' 	=> __( 'The message show at top of email', 'wc_email_inquiry' ),
				'placeholder' 	=> '',
