 ...
 *
 * @class 		WC_Email_Inquiry_New_Account_Email
 * @version		2.1.0
...
class WC_Email_Inquiry_New_Account_Email {
	...
	public static function get_new_account_email_content_html( $email_args = array() ) {
		ob_start();
		$apply_request_a_quote = WC_Email_Inquiry_Quote_Order_Functions::check_apply_request_a_quote();
		$apply_add_to_order = WC_Email_Inquiry_Quote_Order_Functions::check_apply_add_to_order();
		global $wc_ei_orders_mode_new_account_email_settings, $wc_ei_quotes_mode_new_account_email_settings;
		extract($email_args);
		if ( $apply_request_a_quote )
			include ( WC_Email_Inquiry_Quote_Order_Functions::get_template_file_path( $wc_ei_quotes_mode_new_account_email_settings->template_html ) );
		else
			include ( WC_Email_Inquiry_Quote_Order_Functions::get_template_file_path( $wc_ei_orders_mode_new_account_email_settings->template_html ) );
		return ob_get_clean();
	}
	
	public static function get_new_account_email_content_plain( $email_args = array() ) {
		ob_start();
		$apply_request_a_quote = WC_Email_Inquiry_Quote_Order_Functions::check_apply_request_a_quote();
		$apply_add_to_order = WC_Email_Inquiry_Quote_Order_Functions::check_apply_add_to_order();
		global $wc_ei_orders_mode_new_account_email_settings, $wc_ei_quotes_mode_new_account_email_settings;
		extract($email_args);
		if ( $apply_request_a_quote )
			include ( WC_Email_Inquiry_Quote_Order_Functions::get_template_file_path( $wc_ei_quotes_mode_new_account_email_settings->template_plain ) );
		else
			include ( WC_Email_Inquiry_Quote_Order_Functions::get_template_file_path( $wc_ei_orders_mode_new_account_email_settings->template_plain ) );
		return ob_get_clean();
	}
	...
	public static function get_email_content( $email_args=array() ) {
		...
		if ( ($apply_request_a_quote && $wc_email_inquiry_quote_new_account_email_settings['quote_new_account_email_type'] == 'plain') || ($apply_add_to_order && $wc_email_inquiry_order_new_account_email_settings['order_new_account_email_type'] == 'plain' ) ) {
			$email_content = preg_replace( WC_Email_Inquiry_New_Account_Email::$plain_search, WC_Email_Inquiry_New_Account_Email::$plain_replace, strip_tags( WC_Email_Inquiry_New_Account_Email::get_new_account_email_content_plain($email_args) ) );
		} else {
			$email_content = WC_Email_Inquiry_New_Account_Email::style_inline( WC_Email_Inquiry_New_Account_Email::get_new_account_email_content_html($email_args) );
		}
	...
	public static function get_header() {		
		return "Content-Type: " . WC_Email_Inquiry_New_Account_Email::get_content_type() . "\r\n";
	}
	...
	public static function get_content_type() {
		...
		if ( $apply_request_a_quote ) $new_account_email_type = $wc_email_inquiry_quote_new_account_email_settings['quote_new_account_email_type'];
		else $new_account_email_type = $wc_email_inquiry_order_new_account_email_settings['quote_new_account_email_type'];
		
		switch ( $new_account_email_type ) {
			case "html" :
				return 'text/html';
			case "multipart" :
