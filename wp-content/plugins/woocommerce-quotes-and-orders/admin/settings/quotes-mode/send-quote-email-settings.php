...
class WC_EI_Quotes_Mode_Send_Quote_Email_Settings extends WC_Email_Inquiry_Admin_UI
{
	...
	public function __construct() {
		...
		$wc_db_version = get_option( 'woocommerce_db_version', null );
		if ( version_compare( $wc_db_version, '2.5.0', '>=' ) ) {
			$this->template_html 	= 'emails/send-quote_2.5.php';
			$this->template_plain 	= 'emails/plain/send-quote_2.5.php';
		} elseif ( version_compare( $wc_db_version, '2.4.0', '>=' ) ) {
			$this->template_html 	= 'emails/send-quote_2.4.php';
		}
		...
		add_action( $this->plugin_name . '_settings_' . 'quote_send_quote_email_table' . '_start', array( $this, 'add_email_template_shortcodes' ) );
	...
	public function after_save_settings() {
		if ( ! empty( $_GET['send_quote_move_template'] ) && ( $template = esc_attr( basename( $_GET['send_quote_move_template'] ) ) ) ) {
		...
		if ( ! empty( $_GET['send_quote_delete_template'] ) && ( $template = esc_attr( basename( $_GET['send_quote_delete_template'] ) ) ) ) {
				if ( ! empty( $this->$template ) ) {
	...
	public function add_email_template_shortcodes() {
	?>
    		<tr valign="top">
		    	<th class="titledesc" scope="row" height="0"></th>
		    	<td class="forminp" height="0"></td>
                <td rowspan="3">
                    	<?php _e( 'Send quote email template available shortcodes', 'wc_email_inquiry' ); ?>:<br />
                        <?php
