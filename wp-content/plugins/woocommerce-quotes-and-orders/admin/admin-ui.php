...
class WC_Email_Inquiry_Admin_UI
{
	...
	public $google_api_key_option = 'wc_orders_quotes_google_api_key';

	public $toggle_box_open_option = 'wc_orders_quotes_toggle_box_open';

	public $version_transient = 'a3rev_wc_orders_quotes_update_info';

	public $plugin_option_key = 'a3rev_wc_orders_quotes_plugin';

	public $support_url = 'https://a3rev.com/forums/forum/woocommerce-plugins/quotes-and-orders/';
	 ...
	 */
	public $class_name = 'WC_Email_Inquiry';
	...
	public function get_premium_video_data() {
		$premium_video_data = array(
				...
				'image_url'    => WC_EMAIL_INQUIRY_IMAGES_URL. '/video.jpg',
	...
	public function upgrade_top_message( $echo = false, $setting_id = '' ) {
		$upgrade_top_message = sprintf( '<div class="pro_feature_top_message">'
			. __( 'Advanced Settings - Upgrade to the <a href="%s" target="_blank">%s License</a> to activate these settings.', 'wc_email_inquiry' )
			. '</div>'
			...
			, apply_filters( $this->plugin_name . '_' . $setting_id . '_pro_version_name', apply_filters( $this->plugin_name . '_pro_version_name', __( 'Pro Version', 'wc_email_inquiry' ) ) )
		);
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'wc_email_inquiry' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'wc_email_inquiry' ), $this->support_url );
			} else {
				...
				if ( FALSE !== stristr( $version_transient, '||' )
					...
					&& version_compare( get_option('a3rev_wc_orders_quotes_version') , $version_info[0], '<' ) ) {

						$version_message = sprintf( __( 'There is a new version <span class="a3rev-ui-new-plugin-version">%s</span> available, <a href="%s" target="_blank">update now</a> or download direct from <a href="%s" target="_blank">My Account</a> on a3rev.com', 'wc_email_inquiry' ),
							$version_info[0],
							wp_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' . WC_EMAIL_INQUIRY_NAME ), 'upgrade-plugin_' . WC_EMAIL_INQUIRY_NAME ),
