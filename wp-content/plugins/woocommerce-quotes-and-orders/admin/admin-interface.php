		...
		if ( isset( $_REQUEST['type'] ) ) {
			switch ( trim( $_REQUEST['type'] ) ) {
				...
				case 'check_new_version':
					...
					$new_version = '';
					if ( class_exists( 'WC_Email_Inquiry_Upgrade' ) ) {
						$new_version = WC_Email_Inquiry_Upgrade::get_version_info();
					}

					$version_message = $this->get_version_message();
					$has_new_version = 1;
					if ( '' == trim( $version_message ) ) {
						$version_message = __( 'Great! You have the latest version installed.', 'wc_email_inquiry' );
						$has_new_version = 0;
					} else {
			...
			switch( $value['type'] ) {
				...
				case 'google_api_key':
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo $tip; ?>
							<label for="<?php echo $this->google_api_key_option; ?>"><?php echo __( 'Google Fonts API', 'wc_email_inquiry' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							<input
								...
								/> <span class="description" style="margin-left:5px;"><?php echo __( 'ON to connect to Google Fonts API and have auto font updates direct from Google.', 'wc_email_inquiry' ); ?></span>

							<div>&nbsp;</div>
							<div class="a3rev-ui-google-api-key-container" style="<?php if( 1 != $google_api_key_enable ) { echo 'display: none;'; } ?>">
								<div class="a3rev-ui-google-api-key-description"><?php echo sprintf( __( "Enter your existing Google Fonts API Key below. Don't have a key? Visit <a href='%s' target='_blank'>Google Developer API</a> to create a key", 'wc_email_inquiry' ), 'https://developers.google.com/fonts/docs/developer_api#APIKey' ); ?></div>
								<div class="a3rev-ui-google-api-key-inside 
									<?php
									if ( $wc_ei_fonts_face->is_valid_google_api_key() ) {
									...
									<input
		                                ...
		                                placeholder="<?php echo __( 'Google Fonts API Key', 'wc_email_inquiry' ); ?>"
										<?php echo implode( ' ', $custom_attributes ); ?>
										/>
									<p class="a3rev-ui-google-valid-key-message"><?php echo __( 'Your Google API Key is valid and automatic font updates are enabled.', 'wc_email_inquiry' ); ?></p>
									<p class="a3rev-ui-google-unvalid-key-message"><?php echo __( 'Please enter a valid Google API Key.', 'wc_email_inquiry' ); ?></p>
								</div>
							</div>
						</td>
					</tr><?php
				...
				case 'manual_check_version':
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo $tip; ?>
							<label><?php echo __( 'Check New Version', 'wc_email_inquiry' ); ?></label>
						</th>
						<td class="forminp forminp-manual_check_version">
							...
							<input
                                ...
                                type="button"
								value="<?php echo __( 'Check Now', 'wc_email_inquiry' ); ?>"
				...
				case 'onoff_toggle_box' :
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo $tip; ?>
							<label for="<?php echo $this->toggle_box_open_option; ?>"><?php echo __( 'Open Box Display', 'wc_email_inquiry' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							<input
								...
								<?php echo implode( ' ', $custom_attributes ); ?>
								/> <span class="description" style="margin-left:5px;"><?php echo __( 'ON and each admin panel setting box OPEN | CLOSED position are saved each time changes are SAVED.', 'wc_email_inquiry' ); ?></span>
                        </td>
					</tr><?php
				break;
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							<input
									...
									checked_label="<?php _e( 'ON', 'wc_email_inquiry' ); ?>"
									unchecked_label="<?php _e( 'OFF', 'wc_email_inquiry' ); ?>"
									type="checkbox"
									value="1"
				...
				case 'upload':
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
                        	<?php echo $description; ?>
                        	<?php echo $wc_ei_uploader->upload_input( $name_attribute, $id_attribute, $option_value, $attachment_id, $value['default'], esc_html( $value['name'] ), $class, esc_attr( $value['css'] ) , '', $strip_methods );?>
						</td>
					</tr><?php
