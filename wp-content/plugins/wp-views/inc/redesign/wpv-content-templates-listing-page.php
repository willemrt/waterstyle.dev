...
function wpv_admin_menu_content_templates_listing_page()
{
    ...
    <div class="wrap toolset-views">
        ...
        <div class="wpv-views-listing-page">
            ...
            if ( 'name' == $arrange_by ) {
                wpv_admin_content_template_listing_name( $search_term );
            } else {
 ...
 * @since unknown
 */
function wpv_admin_content_template_listing_name( $search_term ) {
		...
		if ( $wpv_count_posts == 0 ) {
			...
			if ( $is_search ) {
				if ( $wpv_args['post_status'] == 'trash' ) {
                    // Searching in trash
                    printf(
                        '<div class="wpv-views-listing views-empty-list"><p>%s</p><p><a class="button-secondary" href="%s">%s</a></p></div>',
