...
// class
if ( file_exists( WPV_PATH_EMBEDDED . '/toolset/toolset-common/visual-editor/editor-addon-generic.class.php')
	&& ! class_exists( 'Editor_addon_parametric' )  ) {

	require_once( WPV_PATH_EMBEDDED . '/toolset/toolset-common/visual-editor/editor-addon-generic.class.php' );
	...
	class Editor_addon_parametric extends Editor_addon_generic {
		...
		public function register_assets() {
			wp_register_script( 'knockout2', WPV_URL . '/res/js/redesign/lib/knockout-2.2.1.debug.js', array(), '2.2.1' );
			 ...
			 */
			wp_register_script( 'knockout-sortable', WPV_URL . '/res/js/lib/knockout-sortable.min.js', array( 'jquery', 'knockout2' ), '0.11.0' );
			...
			if ( ! wp_style_is( 'toolset-notifications-css', 'registered' ) ) {
				wp_register_style( 'toolset-notifications-css', WPV_URL_EMBEDDED . '/toolset/toolset-common/utility/css/notifications.css', array(), WPV_VERSION );
			}
		}
		...
		public function getPostTypesMetas( $object_types, $include_hidden = false ) {
			...
			if( isset( $metaKeysAll['error'] ) ) {
				return array( 'error' => $metaKeysAll['error'] );
			}
		...
		private function formGenericDataStore( $data = array( ) ) {
			if( !is_array( $data ) ) {
				return json_encode( array( 'error' => __( sprintf( 'Parameter should be an array %s', __METHOD__ ), 'wpv-views' ) ) );
			}
		...
		{
			if( !is_array( $args ) ) {
				return array( "error" => __( sprintf( "Not able to build data %s", __METHOD__ ), 'wpv-views') );
			}
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				...
				if( $_POST['fields'] && sizeof( $_POST['fields']['url_param'] ) > 0 ) {
					...
					if( $_POST['fields']['kind'] == 'taxonomy' ) {
						...
						$mode = 'slug';
						$settings[ str_replace( 'taxonomy-', 'tax_', $base_string ) . '_relationship'] =
								isset( $settings[ $base_string . '_relationship' ] )
										? $settings[ $base_string . '_relationship' ]
										: 'FROM URL';
						...
						$settings[ $base_string . '-attribute-url-format' ] =
								isset( $settings[ $base_string . '-attribute-url-format' ] )
										? $settings[ $base_string . '-attribute-url-format' ]
										: array( 'slug' );
						...
						$settings[ $base_string . '_value' ] =
								( sizeof( $_POST['fields']['url_param'] ) == 1 )
										? 'URL_PARAM(' . $_POST['fields']['url_param'][0]['value'] . ')'
										: 'URL_PARAM(' . $_POST['fields']['url_param'][0]['value'] . '), URL_PARAM(' . $_POST['fields']['url_param'][1]['value'] . ')';
						...
						$settings['post_relationship_shortcode_attribute'] =
								isset( $settings['post_relationship_shortcode_attribute'] )
										? $settings['post_relationship_shortcode_attribute'] : 'wpvprchildof';
					}

					//	$settings['filter_controls_key'][ isset( $settings['filter_controls_key'] ) ? sizeof( $settings['filter_controls_key'] ) : 0 ] = $_POST['fields']['field'];
					$settings['filter_controls_field_name']
							[
								isset( $settings['filter_controls_field_name'] ) ? sizeof( $settings['filter_controls_field_name'] ) : 0
							]
							= $_POST['fields']['field'];

					$settings['filter_controls_mode']
							[
								isset( $settings['filter_controls_mode'])  ? sizeof( $settings['filter_controls_mode'] ) : 0
							]
							= $mode;

					$settings['filter_controls_field_name']
							[
								isset( $settings['filter_controls_field_name'] ) ? sizeof( $settings['filter_controls_field_name'] ) - 1 : 0
							]
							= $_POST['fields']['field'];

					$settings['filter_controls_label']
							[
								isset( $settings['filter_controls_label'] ) ? sizeof( $settings['filter_controls_label'] ) : 0
							]
							= ucfirst( str_replace( '_', ' ', $_POST['fields']['name'] ) );

					$settings['filter_controls_type']
							[
								isset( $settings['filter_controls_type'] ) ? sizeof( $settings['filter_controls_type'] ) : 0
							]
							= $_POST['fields']['type'];

					$values = isset( $_POST['fields']['values'] ) ? $_POST['fields']['values'] : array();

					$settings['filter_controls_values']
							[
								isset( $settings['filter_controls_values'] ) ? sizeof( $settings['filter_controls_values'] ) : 0
							]
							= json_encode( array(
									'values' => $values,
					...
					$settings['filter_controls_enable']
							[
								isset( $settings['filter_controls_enable'] ) ? sizeof( $settings['filter_controls_enable'] ) : 0
							]
							= true;
					...
					$settings['filter_controls_param']
							[
								isset( $settings['filter_controls_param'] ) ? sizeof( $settings['filter_controls_param'] ) : 0
							]
							= $param;
		...
		*
		* Adding the parametric search buttons to the Filter HTML textarea toolbar
		*
		* @param string $textarea
		*
		* @return echo the parametric search buttons
		*
		* @since unknown
		*/
		public function add_parametric_search_buttons( $textarea = '' ) {
			...
			$addon_button = sprintf(
					'<button class="button-secondary parametric-button-open js-code-editor-toolbar-button js-button_%s js-parametric-open-window">
						<i class="%s"></i><span class="button-label">%s</span>
					</button>',
					$this->name,
					$this->media_button_class  . ' fa fa-' . $this->name,
					$this->button_text );
			// generate output content
			$out = sprintf(
					'<li class="editor-parametric-button-wrapper js-editor-addon-button-wrapper">%s</li>',
					$addon_button );
			echo $out;
		}
