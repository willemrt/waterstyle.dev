...
var WPV_ParametricFilterWindow = function() {
	...
	var toogle_buttons_visibility = function( buttons, t, append ) {
		...
		button.empty().append('<i class="icon-expand-alt fa fa-plus-square-o"></i> More controls');
		if ( append ) {
			cont.append(button);
		...
		button.on( 'click', function( event ) {
			...
			if ( buttons_visible ) {
				buttons_visible = false;
				jqp( this ).empty().append('<i class="icon-expand-alt fa fa-plus-square-o"></i> More controls');
			} else if ( ! buttons_visible ) {
				buttons_visible = true;
				jqp( this ).empty().append('<i class="icon-collapse-alt fa fa-minus-square-o"></i> Less controls');
			}
		});
	};
		...
		toggle_slide_up_down_advanced:function(  )
		{
			...
			if( !Advanced_visible.viz )
			{
				jqp('.js-hidden-fields-container').slideDown(WPV_ParametricFilterWindow.FADE_FAST, function(){
					...
					caret.toggleClass( 'icon-caret-down icon-caret-up fa-caret-down fa-caret-up' );
...
var WPV_ParametricSearchButton = function() {
	...
	icon = jqp('<i class="icon-search fa fa-search" />'),
	title = WPV_Parametric.add_toolbar_search_button_title,
	icon_complete = jqp('<i class="icon-bookmark fa fa-bookmark flow-complete js-ps-button-complete" style="display:none" />'),
	title_complete = WPV_Parametric.add_toolbar_search_button_title_complete,
	icon_filter_missing = jqp('<i class="icon-bookmark fa fa-bookmark flow-warning js-ps-button-filter-missing" style="display:none" />'),
...
var WPV_ParametricSpinnerButton = function() {
	...
	icon = jqp('<i class="icon-spinner fa fa-spinner" />'),
	title = WPV_Parametric.add_toolbar_spinner_button_title,
	icon_flag = jqp('<i class="icon-bookmark fa fa-bookmark flow-complete js-ps-button-flag" style="display:none" />'),
...
var WPV_ParametricResetButton = function() {
	...
	icon = jqp('<i class="icon-eraser fa fa-eraser" />'),
...
var WPV_ParametricSubmitButton = function() {
	...
	icon = jqp('<i class="icon-chevron-right fa fa-chevron-right" />'),
	...
	icon_incomplete = jqp('<i class="icon-bookmark fa fa-bookmark flow-warning js-ps-button-incomplete" style="display:none" />'),
	title_incomplete = WPV_Parametric.add_toolbar_submit_button_title_incomplete,
	icon_irrelevant = jqp('<i class="icon-bookmark fa fa-bookmark flow-info js-ps-button-irrelevant" style="display:none" />'),
	...
	self.handle_flags = function() {
		var update_mode = '';
		if ( jqp( '.js-wpv-dps-ajax-results:checked' ).length > 0 ) {
			update_mode = jqp( '.js-wpv-dps-ajax-results:checked' ).val();
