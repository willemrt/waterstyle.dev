		...
		if ( isset( $this_module_data[_VIEWS_MODULE_MANAGER_KEY_] ) && is_array( $this_module_data[_VIEWS_MODULE_MANAGER_KEY_] ) ) {
			...
			if ( $view_pairs ) {
				...
				foreach ( $view_pairs as $view_data ) {
					...
					echo '<li class="views-element"><a href="' . admin_url() . 'admin.php?page=' . $prefix . '-' . $suffix . '&view_id=' . $view_data->ID . '"><i class="icon-views fa fa-wpv-custom ont-icon-19 ont-color-orange"></i>' . $view_data->post_title . '</a></li>';
				}
			}
		}
		if ( isset( $this_module_data[_VIEW_TEMPLATES_MODULE_MANAGER_KEY_] ) && is_array( $this_module_data[_VIEW_TEMPLATES_MODULE_MANAGER_KEY_] ) ) {
			...
			if ( $template_pairs ) {
				foreach ( $template_pairs as $template_data ) {
					if ( $embedded ) {
						echo '<li class="views-element"><a href="' . admin_url() . 'admin.php?page=view-templates-embedded&view_id=' . $template_data->ID . '"><i class="icon-views fa fa-wpv-custom ont-icon-19 ont-color-orange"></i>' . $template_data->post_title . '</a></li>';
					} else {
                        // We have full Views, so we can safely use the WPV_CT_EDITOR_PAGE_NAME constant.
						printf(
                            '<li class="views-element"><a href="%s"><i class="icon-views fa fa-wpv-custom ont-icon-19 ont-color-orange"></i>%s</a></li>',
                            add_query_arg(
...
function wpv_register_modules_sections( $sections ) {
	$sections[_VIEW_TEMPLATES_MODULE_MANAGER_KEY_] = array(
        ...
        'icon_css' => 'icon-views-logo fa fa-wpv-custom ont-icon-16 ont-color-orange'
	);
