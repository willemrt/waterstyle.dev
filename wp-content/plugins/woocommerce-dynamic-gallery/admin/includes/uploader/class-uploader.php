...
-----------------------------------------------------------------------------------*/
class WC_Dynamic_Gallery_Uploader extends WC_Dynamic_Gallery_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $attachment_id = 0, $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $strip_methods = true ) {
		...
		$output .= '<input id="upload_'.$id_attribute.'" class="a3rev-ui-upload-button a3_upload_button button" type="button" value="'.__( 'Upload', 'woo_dgallery' ).'" /> '.$description;
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'woo_dgallery' );
...
global $wc_dgallery_uploader;
$wc_dgallery_uploader = new WC_Dynamic_Gallery_Uploader();
